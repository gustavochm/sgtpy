...
def Apol(rho, eta, T, eps, anij, bnij, cnijk, muad2, npol, sigma3):
    ...
    #beta = 1/(kb * T)
    #diameter = eos.d(beta)
    #eta, _ = eos.eta_bh(rhom, diameter)
...
def d2Apol_drho(rho, eta, deta, T, eps, anij, bnij, cnijk, muad2, npol, sigma3):
    #rhom = rho * Na
    #beta = 1/(kb * T)
    #diameter = eos.d(beta)
    #eta, deta = eos.eta_bh(rhom, diameter)
    ...
    epsa = eps / T / kb
    J2Dij, dJ2Dij, d2J2Dij = np.tensordot(etavec, anij + bnij * epsa, axes = ((1), (0)))
