...
def tpd_obj(alpha, temp_aux, P, di, z_notzero, model, state):
    nc = model.nc
    W = np.zeros(nc)
    W[z_notzero] = alpha**2/4.  # change from alpha to mole numbers
    w = W/W.sum()  # change to mole fraction
    global vgw
    out = model.logfugef_aux(w, temp_aux, P, state, vgw)
    logfugW, vgw, Xassgw = out

    dtpd = np.log(W[z_notzero]) + logfugW[z_notzero] - di
    tpdi = np.nan_to_num(W[z_notzero]*(dtpd-1.))
    tpd = 1. + np.sum(tpdi)
    dtpd *= alpha/2
    return tpd, dtpd
...
def tpd_min(W, Z, T, P, model, stateW, stateZ, vw=None, vz=None, Xassw=None,
    ...
    di = np.log(Z[z_notzero]) + logfugZ[z_notzero]

    alpha0 = 2*W**0.5
    alpha0[alpha0 < 1e-2] = 1e-2
    ...
    alpha = minimize(tpd_obj, alpha0, jac=True, method='BFGS',
                     args=(temp_aux, P, di, z_notzero, model, stateW))
    W = np.zeros(nc)
    W[z_notzero] = alpha.x**2/4.
    w = W/W.sum()
    tpd = alpha.fun
