...
from .ieal import daideal_dxrho
...
class saftvrmie_mix():
    ...
    def __init__(self, mixture):
        ...
        epsouter = np.multiply.outer(self.eps, self.eps)
        self.epsij = np.sqrt(np.multiply.outer(sigma3, sigma3)*epsouter)
        ...
        # Chain contribution necessary terms
        ...
        if polar_bool:
            ...
            msijk = np.cbrt(np.multiply.outer(self.ms,
                            np.outer(self.ms, self.ms)))
            msij[msij > 2.] = 2.
            msijk[msijk > 2.] = 2.
    ...
    def ares(self, x, rho, T):
        ...
        aHS = ahs(xhi)  # Just Value
        a1m = a1(xs, a1ij)  # Just value
    ...
    def dares_drho(self, x, rho, T):
        ...
        # Defining xhi0 without x dependency
        di03 = np.power.outer(dii, np.arange(4))
        xhi00 = dxhi00_drho * rho
        ...
        # To use in A1 and A2 for chain term
        suma1_chain = 1.*suma1_monomer[:, diag_index[0], diag_index[1]]
        suma2_chain = 1.*suma2_monomer[:, diag_index[0], diag_index[1]]
    ...
    def dares_dx(self, x, rho, T):
        ...
        a3m, da3m = da3_dx(xs, dxs_dx, xhixm, dxhixm_dx, self.epsij, self.f4,
                           self.f5, self.f6)
        ...
        # To use with a1 and a2 for chain
        suma1_chain = 1.*suma1_monomer[:, diag_index[0], diag_index[1]]
        suma2_chain = 1.*suma2_monomer[:, diag_index[0], diag_index[1]]
        ...
        if self.polar_bool:
            ...
            a, dax = dApolar_dx(rho, x, T, self.anij, self.bnij, self.cnij,
                                eta, deta_dx, self.eps, self.epsij,
                                self.sigma3, self.sigmaij3, self.sigmaijk3,
                                self.npol, self.mupolad2)
            ares += a
            daresx += dax
    ...
    def dares_dxrho(self, x, rho, T):
        ...
        a2m, da2m = da2_dxxhi(xs, dxs_dx, khs, dkhs, dkhsx, xhixm,
                              ...
                              suma2_monomerx, self.epsij, self.f1,
                              self.f2, self.f3)
        ...
        # for use in a1 and a2 in chain term
        suma1_chain = 1.*suma1_monomer[:, diag_index[0], diag_index[1]]
        suma2_chain = 1.*suma2_monomer[:, diag_index[0], diag_index[1]]
        ...
        da1x = da1xxhi[0]*x0_a1[0, diag_index[0], diag_index[1]]
        da1c += da1xxhi[1]*x0_a1[1, diag_index[0], diag_index[1]]
        da1x *= self.C
        ...
        g2m, dg2m = dg2mca_dxxhi(xhi00, khs, dkhs, dkhsx, xm, self.ms, da2new,
                                 d2anew, da2newx, suma_g2, suma_g2x, self.eps,
                                 dii)
